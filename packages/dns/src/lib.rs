...
#[napi]
pub fn resolve_addr(addr: DnsOptions,options: Option<CommonOptions>) -> Vec<DnsResponseRecords> {
  ...
  let address = socket_addr.parse().unwrap();
  let conn = UdpClientConnection::new(address).unwrap();
  let client = SyncClient::new(conn);

  let name = Name::from_str(addr_name.as_str()).unwrap();
